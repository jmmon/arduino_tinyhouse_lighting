void btnRelease(uint8_t ii, uint8_t bb) {
    // happens after a short delay following a button release
    sectionC[ii]._btnC[bb]->timeReleased = 0;

    if (sectionC[ii]._btnC[bb]->isHeld) {  // runs once if the button was released from a hold
        sectionC[ii]._btnC[bb]->isHeld = false;
    
        // if max brightness was hit last button release, enable extended fade for next fade
        if (bb == 1) { // top (fade up)
            sectionC[ii].enableExtendedFade();
        }
    
    } else { // else this runs if it was not being held (regular press and release)

        if (sectionC[ii]._btnC[bb]->pressCt > MAX_PRESS_COUNT) 
            sectionC[ii]._btnC[bb]->pressCt = MAX_PRESS_COUNT;

        if (DEBUG) {
            if (bb == 1) 
                Serial.print(F(" TOP "));
            else
                Serial.print(F(" BOT "));
            Serial.print(sectionC[ii]._btnC[bb]->pressCt);
            Serial.println(F(" "));
        }

        if (bb == 1) {  // top button actions
            if (sectionC[ii]._btnC[bb]->pressCt == 3) {
                topAction3presses(ii, bb);
                if (DEBUG) {
                    Serial.println(F("Max Brightness {mode:3}"));
                }
            }else if (sectionC[ii]._btnC[bb]->pressCt == 2) {
                topAction2presses(ii);
            }else if (sectionC[ii]._btnC[bb]->pressCt == 1) {
                topAction1press(ii);
            }
        } else {  // bottom button actions
            if (sectionC[ii]._btnC[bb]->pressCt == 3) {
                botAction3presses(ii, bb);
                if (DEBUG) {
                    Serial.println(F("Max Brightness {mode:3}"));
                }
            }else if (sectionC[ii]._btnC[bb]->pressCt == 2) {
                botAction2presses(ii);
            }else if (sectionC[ii]._btnC[bb]->pressCt == 1) {
                botAction1press(ii);
            }
        }
    }
    sectionC[ii]._btnC[bb]->pressCt = 0;
}




//*********************************************************************************************************

//                  TOP PRESS ACTIONS
//  These happen after BTN_RELEASE_TIMER time has passed after a button has been released.
//*********************************************************************************************************

// PRESS TOP from   off :   turn on mode0 to default (or to lastBrightness if available)
// PRESS TOP from white / single colors / max brightness:   bump up brightness
// PRESS TOP from   RGB :   pause colorProgress
void topAction1press(uint8_t ii) {
    if (sectionC[ii].isOn) { // from on:
        if (sectionC[ii].mode == (LOW_CYCLE_STARTS_AT + 1) || sectionC[ii].mode == (LOW_CYCLE_STARTS_AT + 2)) { //rgb, rgb
            // pause colorProgress
            sectionC[ii].colorProgress = !sectionC[ii].colorProgress;
        } else if (sectionC[ii].mode == HIGH_CYCLE_STARTS_AT || sectionC[ii].mode == (HIGH_CYCLE_STARTS_AT + 1) || sectionC[ii].mode == (HIGH_CYCLE_STARTS_AT + 2)) { // r, g, b
            sectionC[ii].RGBW[sectionC[ii].mode - SINGLE_COLOR_MODE_OFFSET] += 0.2;
            if (sectionC[ii].RGBW[sectionC[ii].mode-SINGLE_COLOR_MODE_OFFSET] > 1)
                sectionC[ii].RGBW[sectionC[ii].mode-SINGLE_COLOR_MODE_OFFSET] = 1;

        } else {
            // for other modes, cycle up brightness for colors that are on
            sectionC[ii].masterLevel += 0.2;
            if (sectionC[ii].masterLevel > 1)
                sectionC[ii].masterLevel = 1;
        }
        
        sectionC[ii].updateLights();

    } else { // from off:
        // TODO: use last brightness if available, else default brightness
        sectionC[ii].mode = LOW_CYCLE_STARTS_AT;
        sectionC[ii].switchMode();
    }
}


// DOUBLE PRESS TOP: turn on and switch to next mode.
void topAction2presses(uint8_t ii) {
    if (sectionC[ii].mode >= HIGH_CYCLE_STARTS_AT) {     // CYCLE_HIGH
        sectionC[ii].mode ++;
        if (sectionC[ii].mode >= (HIGH_CYCLE_STARTS_AT + NUM_OF_MODES_IN_CYCLE_HIGH))
            sectionC[ii].mode = HIGH_CYCLE_STARTS_AT;

    } else {                                            // CYCLE_LOW
        sectionC[ii].mode ++;
        if (sectionC[ii].mode >= (LOW_CYCLE_STARTS_AT + NUM_OF_MODES_IN_CYCLE_LOW) )
            sectionC[ii].mode = LOW_CYCLE_STARTS_AT;
    }
    sectionC[ii].switchMode();
}

//  TRIPLE PRESS TOP: MAX BRIGHTNESS
// from on/off (any mode): turn on max brightness
void topAction3presses(uint8_t ii, uint8_t bb) {
    sectionC[ii].mode = (LOW_CYCLE_STARTS_AT + 3); // max brightness mode
    sectionC[ii].switchMode();
}




//*********************************************************************************************************

//                  BOTTOM PRESS ACTIONS
//  These happen after BTN_RELEASE_TIMER time has passed after a button has been released.
//*********************************************************************************************************


// PRESS BOT from off: turn on nightlight mode (red and/or ww)
// PRESS BOT from on: turn off all lights, reset mode;
void botAction1press(uint8_t n) {
    if (sectionC[n].isOn) { // turn off light
        if (DEBUG) {
            Serial.println(F(" BOT 1: Turn Off "));
        }
        //turnOffSection(n);
        sectionC[n].mode = 0;
        sectionC[n].switchMode();
        //add mode for turn off?

    } else { // turn on night light mode
        if (DEBUG) {
            Serial.println(F(" BOT 1: Red "));
        }

        sectionC[n].mode = HIGH_CYCLE_STARTS_AT;
        sectionC[n].switchMode();
    }
}


// DOUBLE PRESS BOT: from on: reduce mode by 1
// DOUBLE PRESS BOT: from off: do nothing
void botAction2presses(uint8_t ii) {
    if (sectionC[ii].isOn) { // do nothing if off
        if (sectionC[ii].mode >= HIGH_CYCLE_STARTS_AT) { // if in the single color modes
            sectionC[ii].mode --;
            if (sectionC[ii].mode < HIGH_CYCLE_STARTS_AT)    // if under 5
                sectionC[ii].mode = (HIGH_CYCLE_STARTS_AT + NUM_OF_MODES_IN_CYCLE_HIGH - 1);   // 5 + 4 - 1 = 8
            
        } else {      // mode is 1-3
            sectionC[ii].mode --;
            if (sectionC[ii].mode < LOW_CYCLE_STARTS_AT) // if under 1
                sectionC[ii].mode = (LOW_CYCLE_STARTS_AT + NUM_OF_MODES_IN_CYCLE_LOW - 1);   //4 - 1 = 3
        } 

        sectionC[ii].switchMode();
    }
}


void botAction3presses(uint8_t n, uint8_t m) {
    bool somethingElseIsOn = false;
    for (uint8_t index = 0; index < SECTION_COUNT; index++) 
        if (sectionC[index].isOn && (index != 2)) 
            somethingElseIsOn = true;

    if (somethingElseIsOn) { // something else on, (porch on or off)
        for (uint8_t index = 0; index < SECTION_COUNT; index++) // turn off everything except porch
            if (sectionC[index].isOn && (index != 2)) {
                //turnOffSection(index);
                sectionC[index].mode = 0;
                sectionC[index].switchMode();
            }
                

    } else {
        if (sectionC[2].isOn) { // nothing else on, porch on
            //turnOffSection(2);
            sectionC[2].mode = 0;
            sectionC[2].switchMode();
        }
        else // DISCO MODE // nothing else on, porch off
            for (uint8_t index = 0; index < SECTION_COUNT; index++) {
                // for each light, switch the mode to (LOW_CYCLE_STARTS_AT + 2) (colorProgress sudden)
                sectionC[index].mode = (LOW_CYCLE_STARTS_AT + 2);
                sectionC[index].switchMode();
            }
    }
}